...
// scriptjs
(function(g,h){this[g]=h()})("$ADSPDBLscript",function(){function g(a,d){for(var b=0,c=a.length;b<c;++b)if(!d(a[b]))return!1;return 1}function h(a,d){g(a,function(a){d(a);return 1})}function c(a,d,b){function r(a){return a.call?a():l[a]}function k(){if(!--q){l[p]=1;f&&f();for(var a in e)g(a.split("|"),r)&&!h(e[a],r)&&(e[a]=[])}}a=a.push?a:[a];var t=d&&d.call,f=t?d:b,p=t?a.join(""):d,q=a.length;setTimeout(function(){h(a,function v(a,b){if(null===a)return k();b||/^https?:\/\//.test(a)||!m||(a=-1===
a.indexOf(".js")?m+a+".js":m+a);if(n[a])return 2==n[a]?k():setTimeout(function(){v(a,!0)},0);n[a]=1;u(a,k)})},0);return c}function u(a,d){var b=p.createElement("script"),c;b.onload=b.onerror=b.onreadystatechange=function(){b.readyState&&!/^c|loade/.test(b.readyState)||c||(b.onload=b.onreadystatechange=null,c=1,n[a]=2,d())};b.async=1;b.src=f?a+(-1===a.indexOf("?")?"?":"&")+f:a;q.insertBefore(b,q.lastChild)}var p=document,q=p.getElementsByTagName("head")[0],l={},e={},n={},m,f;c.get=u;c.order=function(a,
d,b){(function k(e){e=a.shift();a.length?c(e,k):c(e,d,b)})()};c.path=function(a){m=a};c.urlArgs=function(a){f=a};c.ready=function(a,d,b){a=a.push?a:[a];var f=[];!h(a,function(a){l[a]||f.push(a)})&&g(a,function(a){return l[a]})?d():!function(a){e[a]=e[a]||[];e[a].push(d);b&&b(f)}(a.join("|"));return c};c.done=function(a){c([null],a)};return c});
...
(function () {
    ...
    var playerInstance = '';
    var type = "";
    // to store if an Ad is vpaid or vast
    var creativeType='';
    // IOS version
    var iOS = parseFloat(
...
window.aud = tempAudStore[1];
    ...
    var processMessage = function (event) {
        if (event.data == "is in view") {
            ...
            if(!contentplay && !unitClosed && (typeof playerInstance=='object' && playerInstance.hasOwnProperty('getState') && playerInstance.getState() == "paused")){

                  if(typeof playerInstance=='object' && playerInstance.hasOwnProperty('play')){
            ...
            if(!contentplay && !unitClosed && (typeof playerInstance=='object' && playerInstance.hasOwnProperty('getState') && playerInstance.getState() == "playing")){
            ...
            // INTEGRAL JAVASCRIPT SOURCE NOW GOES IN CONFIG FILE in this VARIABLE: iasViewabilityJavascriptSource!!!!
    ...
    // source of vast tag
    // !!!!!!!!!!!!! IMPORTANT -  if you have to replace this tag for some reason DELETE //done off the end of the line
    ...
    if(isMobile.apple.phone && bowser.ios && iOS>=10){
      videoMobileDevices = true;
    }
    ...
    if (contentplay || tubemogul) {
        ...
        if($('img[data-title="Main Image Tag"]').length>0) {
            ...
            $('#container').prepend('<img id="sound-icon" class="s-on" src="https://files.adspdbl.com/assets/sound-off.png" style="width:6.14%;height:13%;position:absolute;right:1%;top:1%;z-index:2000;cursor:pointer;display:none;"/>');
...
// mobile video controls
    ...
    postMessageTracking = function (tracking) {
        var cdmn = '';
        cdmn = completeDomain;
        if(completeDomain.indexOf('netmums')>-1 || completeDomain.indexOf('seattlepi')>-1 || completeDomain.indexOf('beaumontenterprise')>-1 || completeDomain.indexOf('ctpost')>-1 || completeDomain.indexOf('chron.com')>-1 || completeDomain.indexOf('greenwichtimes.com')>-1 || completeDomain.indexOf('mysa.com')>-1 || completeDomain.indexOf('newstimes.com')>-1 || completeDomain.indexOf('sfgate.com')>-1 || completeDomain.indexOf('stamfordadvocate.com')>-1 || completeDomain.indexOf('timesunion.com')>-1 || completeDomain.indexOf('ibtimes.co.uk')>-1){
          if(completeDomain.indexOf('www')<0){
            cdmn = 'www.'+completeDomain;
          }
        }
        // asia online
        if(completeDomain.indexOf('asiaone')>-1){
          cdmn = 'news.'+completeDomain;
        }
        var protocal = (location.protocol=='https:'?'https://':'http://');
        parent.postMessage(tracking, protocal + cdmn);
    ...
    trackingCall = function (info) {
        ...
        // win price
        var winPricePatt = /.*?wip-(.*?)-wip-.*/gim;
        var win_price = winPricePatt.exec(completeHash);
        if(Array.isArray(win_price)){
            win_price = win_price[1];
        }
        else {
            win_price = "";
        }
        ...
        if (state === 'click') {
            ...
            if (isMobile.any) {
                ...
                if (touchCounter < 600) {
                    pSrc = "https://reports.metadsp.co.uk/external-tracking/track.php?redirectAdsp=" + encodeURIComponent(clickRedirect) + "&supplier=spartan-pmp&ran=" + ran + "&type=event&state=" + state + "&audience=" + window.aud + "&campaign=" + campaign + "&spartan_udo_id="+spartanid+"&prc="+ win_price +"&general=" + general + "&click_title=" + clickTitle + "&resolution=" + $(window).width() + "x" + $(window).height() + "&keyword_data=" + keywordStr + "&domain=" + completeDomain + "&path=" + completeUrlformatted;
            ...
            // not mobile
                ...
                pSrc = "https://reports.metadsp.co.uk/external-tracking/track.php?redirectAdsp=" + encodeURIComponent(clickRedirect) + "&supplier=spartan-pmp&ran=" + ran + "&type=event&state=" + state + "&audience=" + window.aud + "&campaign=" + campaign + "&prc="+ win_price +"&spartan_udo_id="+spartanid+"&general=" + general + "&click_title=" + clickTitle + "&resolution=" + $(window).width() + "x" + $(window).height() + "&keyword_data=" + keywordStr + "&domain=" + completeDomain + "&path=" + completeUrlformatted;
            }
        }
        else {
            pSrc = "https://reports.metadsp.co.uk/external-tracking/track.gif?ran=" + ran + "&type=event&state=" + state + "&supplier=spartan-pmp&audience=" + window.aud + "&campaign=" + campaign + "&prc="+ win_price +"&spartan_udo_id="+spartanid+"&general=" + general + "&click_title=" + clickTitle + "&resolution=" + $(window).width() + "x" + $(window).height() + "&keyword_data=" + keywordStr + "&domain=" + completeDomain + "&path=" + completeUrlformatted;
        }
        if (pSrc.length > 2000) {
            ...
            if ($('#video-section').length > 0) {
                ...
                if (typeof playerInstance == 'undefined') {
                    ...
                    tmPlayerReady = function () {
                        if (typeof playerInstance != 'undefined' && (playerInstance.getState() == "paused" || playerInstance.getState() == "idle") || vastadPaused || (creativeType.indexOf('vpaid')>-1 && playerInstance.getState() == "buffering")) {
                            ...
                            if (!isMobile.any || videoMobileDevices) {
                                ...
                                if (contentplay && vastadPaused || (contentplay && creativeType.indexOf('vpaid')>-1 && playerInstance.getState() == "buffering")) {
                                    var vc = $('#video-container video').get(0);
                                    postMessageTracking('eiv');
    ...
    var videoPlayer = function (video, posterImage, videoPrettyName, introVideo) {
        ...
        /*7.7.1*/ jwplayer.key ='/LQDv4UoF/ntTSbKA84sByJCmb/kqjI5TuleNw==';
        ...
        if (tubemogul) {
            ...
            playerInstance = jwplayer('video-container');
            playerInstance.setup({
                // if content card vast:
                // advertising: {"source":"https://etc","client": "vast"},
                playlist: [{
                    image: videoPosterImage,
                    sources: [
                        /*{file: "rtmp://flash.adspdbl.com/cfx/st/mp4:" + campaign + '/' + video},
                         ...
                         {file: "assets/" + (isMobile.apple.device?video.replace('mp4', 'm3u8'):video)},
                ...
                // set autostart to true if its a contentcard unit and you have to click to lauch the video
                autostart: false,
                //androidhls:true,
                primary: (function () {
                    rt = 'html5';
        ...
        if (tubemogul || contentplay) {
            ...
            jwplayer("video-container").on('adImpression',function (e) {
                ...
                postMessageTracking('teaserinview');
        ...
        if (!isMobile.any) {
            // play on desktop
            // new on jwplayer 7.7 ready state!!!
            jwplayer().on('ready',function(){
              // iphone iOS 10
              var video = $('video').get(0);
              makeVideoPlayableInline(video);
        ...
        playerInstance.onTime(function (e) {
            ...
            if(contentplay && !hostedVastVideo && !currentlyExpanded){
                playerInstance.pause();
                return;
            }
            ...
            if (!contentplay) {
                if (!playingUnder5 && mobilePauseClear) {
                    ...
                    if (!tubemogul) {
                        if (videoPrettyName.toLowerCase().indexOf('intro') > -1) {
                          if(!hostedVastVideo){
                            trackingCall('-events-mousepress-evente--soc-vast ad start - ' + videoPrettyName + '-soe--cos--coe-/' + campaign + '/' + getKeywordInfo.info);
                          }
                        }
                        else {
                          if(!hostedVastVideo){
                ...
                //track play - only for intro videos
            ...
            if (pos > firstQ && !firstQreached) {
                ...
                if (!hostedVastVideo && contentplay && type == 'collective') {
                  ...
                  if(!hostedVastVideo){
                    trackingCall('-events-mousepress-evente--soc-vast ad first quartile - ' + videoPrettyName + '-soe--cos--coe-/' + campaign + '/' + getKeywordInfo.info);
                }
            }
            }
            ...
            if (pos > halfWay && !halfReached) {
                ...
                if(!hostedVastVideo && contentplay && type=='collective'){
                  ...
                  if(!hostedVastVideo){
                    trackingCall('-events-mousepress-evente--soc-vast ad halfway - ' + videoPrettyName + '-soe--cos--coe-/' + campaign + '/' + getKeywordInfo.info);
                }
            }
            }
            ...
            if (pos > thirdQ && !thirdQreached) {
                ...
                if(!hostedVastVideo && contentplay && type=='collective'){
                  ...
                  if(!hostedVastVideo){
                    trackingCall('-events-mousepress-evente--soc-vast ad third quartile - ' + videoPrettyName + '-soe--cos--coe-/' + campaign + '/' + getKeywordInfo.info);
                }
            }
            }
            ...
            if (pos >= vidLength - 0.4 && !end_reached && playing && firstQreached) {
                ...
                if(!hostedVastVideo && contentplay && type=='collective'){
                    ...
                    if(!hostedVastVideo){
                      trackingCall('-events-mousepress-evente--soc-vast ad complete - ' + videoPrettyName + '-soe--cos--coe-/' + campaign + '/' + getKeywordInfo.info);
                    }
        ...
        if (tubemogul || contentplay) {
            jwplayer("video-container").on('adTime',function (e) {
                ...
                if (pos>30 && !vast30reached) {
                    vast30reached = true;
                    // vapid doesn't report back length accurately
                    if(creativeType.indexOf('vpaid')<0){
    ...
    window.closeVideo = function (file, duration) {
        ...
        if (!contentplay || contentCardVast) {
            ...
            // is unit closed? or has close video button been clicked. if close video clicked remove video altogether
            if (location.hash.indexOf('SEPSLIDE-stop') < 0) {
                // remove video completely
                playerInstance.stop();

                //******* WARNING - DO NOT PUT AN OPEN VIDEO STATEMENT HERE AS IT WILL CAUSE AN INFINTE VIDEO START LOOP
    ...
    $(document).ready(function () {
        ...
        //custom clicks for index variable for social class;
        /*$('.social').each(function(index,element){
            $(element).attr('data-title',dataTitle[index]);
            $(element).attr('data-link',dataLink[index]);
